...
namespace DS4Windows
{
    ...
    public class DS4Device
    {
        ...
        // ss7
        protected PointF lightgunXY = new PointF(127, 127);
        protected bool lightgunJustReloaded = false;
        ...
        protected unsafe void performDs4Input()
        {
            unchecked
            {
                ...
                while (!exitInputThread)
                {
                    ...
                    // ss7 start
                    //
                    bool lightgunEnabled = Global.GetGyroOutMode(DeviceSlotNumber) == GyroOutMode.Lightgun;

                    SeanstarHelper.LightgunBtnEnum fireBtnEnum = SeanstarHelper.LightgunBtnEnum.NOT_SET;
                    bool allowLX = true;
                    bool allowLY = true;
                    bool allowRX = true;
                    bool allowRY = true;

                    if (lightgunEnabled && sixAxis is { } && sixAxis.NowIsNotNull) {

                        bool hijackLeftAnalog = Global.lightgunAnalog[DeviceSlotNumber] == 0;
                        bool reload = SeanstarHelper.GetLightgunBtnPressed(Global.lightgunReload[DeviceSlotNumber], cState);
                        bool recenter = SeanstarHelper.GetLightgunBtnPressed(Global.lightgunRecenter[DeviceSlotNumber], cState);
                        ...
                        if (recenter) {
                            ...
                            lightgunXY = new PointF(127, 127);
                        }

                        // else if: off-screen reload:
                        else if (reload) {

                            // Move analog stick (gun cursor) off-screen:
                            if (hijackLeftAnalog) {
                                cState.LX = 0;
                                allowLY = true;
                            }
                            ...
                            SeanstarHelper.SetLightgunBtnPressed(Global.lightgunFire[DeviceSlotNumber], cState);
                            fireBtnEnum = SeanstarHelper.GetBtn(Global.lightgunFire[DeviceSlotNumber]);

                            lightgunJustReloaded = true;
                        }

                        // else: move Analog Stick
                        else {

                            float angVelYaw = (float)(sixAxis.now.angVelYaw * Math.PI / 180.0);
                            float angVelPitch = (float)(sixAxis.now.angVelPitch * Math.PI / 180.0);

                            float factor = 2.2f;

                            // Cursor X:
                            angVelYaw *= 1.5f;
                            float accelX = MathF.Sign(angVelYaw) * (float)Math.Log(Math.Abs(angVelYaw) + 1) * factor;

                            float tx = angVelYaw * sixAxis.now.gyroYawFull;
                            if (tx > 1.0f || lightgunJustReloaded) {
                                float newXF = Math.Clamp(lightgunXY.X + accelX, 0, 255);

                                if (hijackLeftAnalog) {
                                    cState.LX = Convert.ToByte(newXF);
                                }
                                else {
                                    cState.RX = Convert.ToByte(newXF);
                                }
                                lightgunXY.X = newXF;
                                if (lightgunJustReloaded) lightgunJustReloaded = false;
                            }

                            // Cursor Y:
                            float ty = angVelPitch * sixAxis.now.gyroPitchFull;
                            if (ty > 1.0f || lightgunJustReloaded) {
                                float accelY = angVelPitch * 1.5f;
                                float newYF = Math.Clamp(lightgunXY.Y - accelY, 0, 255);

                                if (hijackLeftAnalog) {
                                    cState.LY = Convert.ToByte(newYF);
                                }
                                else {
                                    cState.RY = Convert.ToByte(newYF);
                                }
                                lightgunXY.Y = newYF;
                                if (lightgunJustReloaded) lightgunJustReloaded = false;
                            }
                    ...
                    if (allowLX) cState.LX = inputReport[1];
                    if (allowLY) cState.LY = inputReport[2];
                    if (allowRX) cState.RX = inputReport[3];
                    if (allowRY) cState.RY = inputReport[4];
                    ...
                    if (fireBtnEnum != SeanstarHelper.LightgunBtnEnum.TRIANGLE_Y)
                        cState.Triangle = (tempByte & (1 << 7)) != 0;
                    
                    if (fireBtnEnum != SeanstarHelper.LightgunBtnEnum.CIRCLE_B)
                        cState.Circle = (tempByte & (1 << 6)) != 0;

                    if (fireBtnEnum != SeanstarHelper.LightgunBtnEnum.CROSS_A)
                        cState.Cross = (tempByte & (1 << 5)) != 0;

                    if (fireBtnEnum != SeanstarHelper.LightgunBtnEnum.SQUARE_X)
                    ...
                    if (fireBtnEnum != SeanstarHelper.LightgunBtnEnum.R1)
                        cState.R1 = (tempByte & (1 << 1)) != 0;

                    if (fireBtnEnum != SeanstarHelper.LightgunBtnEnum.L1)
